	...
	*/
	...
	class SSP_copy {
		...
		static function complex_new (
			...
			$join = (isset($request['join']) && $request['join'] && !$where) ? 'redaction.`idArticle` = article.`idArticle` AND ' : '';
			$joine = 'redaction.`idArticle` = article.`idArticle` ';
			...
			if ( $whereAll ) {
				...
				$whereAllSql = 'WHERE '.$join.''.$str;
			}
			
			if(!$where && isset($request['join']) && $request['join']) {//--------------------------------------------------------------------------------------------
				$where = 'WHERE '.$joine;
			}//-------------------------------------------------------------------------------------------------------
			...
			// if(isset($request['join']) && $request['join']) {
			// 	$sql = str_replace (
			// 		array('`idArticle`', '`etat`','`nbrMots`'), 
			// 		array('redaction.`idArticle`', 'redaction.`etat`', 'redaction.`nbrMots`'), 
			// 	$sql );
			// }
	
			//test scope-----------------------------------------------------------------
			file_put_contents("debug.json" , json_encode($test));
			//close test scope--------------------------------------------------------------
